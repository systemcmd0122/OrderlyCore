...
function loadCommands() {
    console.log(chalk.blue('ℹ️ Loading commands...'));
    ...
    for (const file of commandFiles) {
        try {
            ...
            if ('data' in command && 'execute' in command) {
                ...
                console.warn(chalk.yellow(`⚠️ Warning: Command at ${filePath} is missing required properties.`));
            }
        } catch (error) {
...
function loadEvents() {
    console.log(chalk.blue('ℹ️ Loading events...'));
...
async function deployCommands(commands) {
    ...
    try {
        console.log(chalk.blue(`ℹ️ Registering ${commands.length} commands...`));
        const data = await rest.put(
            Routes.applicationCommands(process.env.CLIENT_ID),
            { body: commands }
        );
        console.log(chalk.green(`✅ Successfully registered ${data.length} commands.`));
    } catch (error) {
